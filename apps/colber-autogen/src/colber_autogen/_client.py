...
Plugin components (instrumentation wrappers, memory, tools) all accept
an optional ``client`` kwarg. When the caller doesn't pass one, we
build one from environment variables — keeping the AutoGen integration
a one-liner for the common case (Colber stack reachable on localhost or
via a single ingress URL).
