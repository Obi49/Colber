...
def test_reputation_score_returns_envelope(
    ...
    rendered = tool._run(did="did:key:zTarget")
    payload = json.loads(rendered)
...
def test_negotiation_propose_unknown_returns_error_string(
    colber_client: Any,
) -> None:
    propose = NegotiationProposeTool(client=colber_client)
    rendered = propose._run(
...
def test_insurance_subscribe_validation_failure_returns_error(
    colber_client: Any,
) -> None:
    sub = InsuranceSubscribeTool(client=colber_client)
    rendered = sub._run(
