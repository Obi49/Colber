...
- LangChain components (callbacks, memory, tools) accept the test
  client so the public surface can be exercised end-to-end without
  spinning up a Colber stack.

The fake backend is intentionally minimal — just enough to make the
plugin's surface assertable. It mirrors the wire shapes the real
services emit.
"""
...
class FakeColberBackend:
    ...
    def handle(self, request: httpx.Request) -> httpx.Response:
        ...
        # Strip optional `/<service>/` prefix so the dispatcher works
        # whether the SDK is wired with port-per-service URLs or a
        # single ingress URL.
        for prefix in (
            "/identity",
            "/reputation",
    ...
    def _register(self, body: dict[str, Any]) -> httpx.Response:
        ...
        # Derive a deterministic-ish DID from the public key bytes so the
        # caller can re-use the DID it expects (no real did:key derivation
        # — this is a fake backend).
...
def colber_client(fake_backend: FakeColberBackend) -> Iterator[ColberClient]:
    ...
    Retries are turned off and the inter-retry sleep is a no-op so tests
