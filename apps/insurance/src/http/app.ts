...
import { registerInsuranceRoutes } from './routes.js';
...
export const buildApp = async (deps: BuildAppDeps): Promise<FastifyInstance> => {
  ...
  registerHealthRoutes(app, { dbClient: deps.dbClient, reputation: deps.reputation });
  registerInsuranceRoutes(app, {
    service: deps.insurance,
    adminEnabled: deps.adminEnabled,
  });
