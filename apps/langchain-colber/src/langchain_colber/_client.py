...
Plugin components (callback handler, memory, tools) all accept an
optional ``client`` kwarg. When the caller doesn't pass one, we build
one from environment variables — keeping the LangChain integration a
one-liner for the common case (Colber stack reachable on localhost or
