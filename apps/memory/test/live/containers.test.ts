/**
 * Live integration tests against real Postgres + Qdrant + Ollama containers.
 *
 * Skipped by default: this suite is gated behind `PRAXIS_LIVE_TESTS=1` so the
 ...
 *
 *     pnpm --filter @praxis/memory add -D testcontainers @testcontainers/postgresql @testcontainers/qdrant
 *
 * The Ollama side stays out-of-process — point at the praxis-stack docker
 * Ollama or run a temporary local container, since pulling the model image
 * inside a per-test container is too slow.
 *
 * The `describe.skipIf(!process.env.PRAXIS_LIVE_TESTS)` wrapper means the
...
describe.skipIf(!process.env.PRAXIS_LIVE_TESTS)('live (testcontainers)', () => {
  it('is a placeholder — wire up real containers when PRAXIS_LIVE_TESTS=1', () => {
    // Intentionally minimal. The shape of this suite (boot Postgres + Qdrant,
    // run drizzle migrations, exercise the full stack via Fastify.inject) is
    // documented in apps/memory/README.md. Filling it in lives behind a
