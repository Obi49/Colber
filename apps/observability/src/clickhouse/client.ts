...
/** Coerce a ClickHouse JSONEachRow scalar (unknown) to string. */
...
const rowToQueryRow = (request: QueryRequest, raw: Record<string, unknown>): QueryRow => {
  ...
  if (request.scope === 'logs') {
    const out: QueryRow = {
      ...
      spanId: asString(raw.span_id),
      service: asString(raw.service),
      level: asString(raw.level),
      message: asString(raw.message),
      attributes: attr,
      resource,
      ...(raw.parent_span_id ? { parentSpanId: asString(raw.parent_span_id) } : {}),
      ...(raw.agent_did ? { agentDid: asString(raw.agent_did) } : {}),
      ...(raw.operator_id ? { operatorId: asString(raw.operator_id) } : {}),
    };
    return out;
  }
  const out: QueryRow = {
    timestamp: asString(raw.start_timestamp),
    traceId: asString(raw.trace_id),
    spanId: asString(raw.span_id),
    service: asString(raw.service),
    ...
    startTimestamp: asString(raw.start_timestamp),
    endTimestamp: asString(raw.end_timestamp),
    ...
    ...(raw.agent_did ? { agentDid: asString(raw.agent_did) } : {}),
    ...(raw.operator_id ? { operatorId: asString(raw.operator_id) } : {}),
    ...(raw.status_message ? { statusMessage: asString(raw.status_message) } : {}),
  };
  return out;
};
