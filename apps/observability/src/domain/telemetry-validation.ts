...
const requireResource = (raw: unknown, field: string): Record<string, string> | undefined => {
  if (raw === undefined || raw === null) {
    return undefined;
  }
  if (typeof raw !== 'object' || Array.isArray(raw)) {
...
export const validateSpanEvent = (raw: unknown): SpanEvent => {
  ...
  // Check timestamp ordering BEFORE validating durationMs so that a malformed
  // span (end < start) yields a precise "endTimestamp ..." error instead of a
  // generic "durationMs must be non-negative" message — better UX for API
  // consumers debugging clock skew or wrong field assignment.
