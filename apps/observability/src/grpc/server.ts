...
import type { AlertScope } from '../domain/alert-types.js';
...
const decodeFilter = (msg: QueryFilterMsg): QueryFilter => {
  ...
  return { field: msg.field, op: msg.op as QueryFilter['op'], value: value as QueryFilter['value'] };
};
  ...
  const handlers: ObservabilityGrpcService = {
    ...
    UpdateAlert: (call, callback) => {
      try {
        ...
        if (r.set_enabled === true) {
          patch.enabled = r.enabled_value === true;
        }
        ...
        if (r.condition) {
          const conditionFilters = (r.condition.filters ?? []).map(decodeFilter);
          patch.condition = {
            operator: r.condition.operator === 'or' ? 'or' : 'and',
            filters: conditionFilters,
            windowSeconds: r.condition.window_seconds ?? 60,
            threshold: r.condition.threshold ?? 1,
          };
        }
        if (typeof r.cooldown_seconds === 'number') {
          patch.cooldownSeconds = r.cooldown_seconds;
        }
        if (r.notification) {
          const channels = (r.notification.channels ?? []).map((c) => ({
            type: c.type,
          ...
          patch.notification = { channels: channels as never };
        }
        service
...
const alertToProto = (a: import('../domain/alert-types.js').AlertRule): AlertProto => ({
  id: a.id,
