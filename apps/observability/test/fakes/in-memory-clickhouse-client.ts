...
import type { QueryRequest, QueryRow } from '../../src/domain/query-types.js';
...
const compare = (
  actual: unknown,
  op: import('../../src/domain/alert-types.js').FilterOperator,
  expected: unknown,
): boolean => {
  switch (op) {
    case 'eq':
    ...
    case 'matches':
      return (
        typeof actual === 'string' && typeof expected === 'string' && new RegExp(expected).test(actual)
      );
    default:
      return false;
  }
...
const matchesLog = (
  l: LogEvent,
  filter: import('../../src/domain/query-types.js').QueryFilter,
): boolean => compare(fieldValueLog(l, filter.field), filter.op, filter.value);

const matchesSpan = (
  s: SpanEvent,
  filter: import('../../src/domain/query-types.js').QueryFilter,
): boolean => compare(fieldValueSpan(s, filter.field), filter.op, filter.value);
