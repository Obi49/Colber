...
import {
  validateLogEvent,
  validateSpanEvent,
} from '../../src/domain/telemetry-validation.js';
...
describe('validateLogEvent', () => {
  ...
  it('rejects an invalid traceId', () => {
    expect(() => validateLogEvent({ ...baseLog, traceId: 'too-short' })).toThrowError(
      /traceId/,
    );
  });
...
describe('validateSpanEvent', () => {
  ...
  it('rejects unknown status', () => {
    expect(() => validateSpanEvent({ ...baseSpan, status: 'whatever' })).toThrowError(
      /status/,
    );
  });
