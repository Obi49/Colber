# STATUS — Colber (snapshot pause)

**Date** : 2026-05-01 (rebrand)
**Phase** : **v1 PLATFORM COMPLÈTE — 5/5 modules + rebrand Colber**
**Branche active** : `claude/kind-mccarthy-d06ab7` (worktree, à merger sur `main`)
**Dernier commit fonctionnel pushé** : `4c72638` — rebrand technique en cours

---

## 0. Rebrand Praxis → Colber (2026-05-01)

Le projet a été renommé **Colber** définitivement (anciennement _Praxis_ depuis avril 2026, anciennement _AgentStack_ dans le CDC v1.0 figé). Domaine officiel acquis : **`colber.dev`**.

**Ce qui a changé techniquement :**

- Tous les packages npm `@praxis/*` → `@colber/*` (5 packages core + 6 apps).
- Variables d'environnement `PRAXIS_*` → `COLBER_*`.
- Bases Postgres `praxis_*` → `colber_*` (à recréer côté VM β — non-rétrocompatible).
- DB ClickHouse `praxis` → `colber`, tables `praxis_logs/spans` → `colber_logs/spans`.
- Conteneurs et images Docker `praxis-*` / `praxis/*` → `colber-*` / `colber/*`.
- Projet compose `-p praxis` → `-p colber`.
- Namespaces gRPC `praxis.<service>.v1` → `colber.<service>.v1`.
- Dossier `praxis-stack/` → `colber-stack/` (via `git mv`, historique préservé).
- 4 fichiers Mermaid `docs/diagrams/praxis-*.md` → `colber-*.md`.
- `LICENSE` Apache-2.0 ajouté (était `UNLICENSED`).
- `NOTICE` ajouté avec historique des renommages.

**Ce qui n'a PAS changé** (volontairement) :

- `docs/AgentStack_Cahier_des_charges.docx` (CDC v1.0 figé, ancien nom préservé).
- Repo GitHub : reste `Obi49/Praxis` jusqu'à ce que le CdP fasse le rename manuellement (GitHub conserve les redirects 301).
- VM β `100.83.10.125` : conteneurs `praxis-*` toujours en cours, à recréer en `colber-*` au prochain déploiement.

**Action manuelle CdP requise :**

1. Renommer le repo GitHub `Obi49/Praxis` → `Obi49/Colber`.
2. Sur la VM β : `docker compose -p praxis down -v` puis pull du nouveau repo et `docker compose -p colber up -d` (recrée les bases avec les nouveaux noms).
3. Mettre à jour `praxis-stack/services.env` (sur la VM uniquement) → `colber-stack/services.env`.
...
- **Endpoints** :
  ...
  - gRPC : `colber.identity.v1.IdentityService`
...
- **Storage** : Postgres `colber_identity`, table `agents (did, public_key, signature_scheme, owner_operator_id, registered_at, revoked_at)`.
- **Tests** : 21 (unit + integration via fastify.inject).
- **Image Docker** : `colber/agent-identity:dev` (294 MB, Alpine multi-stage).
...
- **Endpoints** :
  ...
  - gRPC : `colber.reputation.v1.ReputationService`
...
- **Image Docker** : `colber/reputation:dev` (317 MB).
...
- **Endpoints** :
  ...
  - gRPC : `colber.observability.v1.ObservabilityService` (logs / traces / query + CRUD alerts)
  - MCP : `observability.log`, `observability.trace`, `observability.query`, `observability.alert`
- **Storage** :
  - ClickHouse 24.10 : 2 tables (`colber_logs`, `colber_spans`) auto-créées au démarrage, partitionnement par jour, ORDER BY `(timestamp, traceId, spanId)`, TTL 30 jours configurable.
  - Postgres `colber_observability` (migration drizzle 0000) : table `alert_rules` (id, ownerOperatorId, name, scope, condition jsonb, cooldownSeconds, notification jsonb).
...
- **Tests** : 32 unit (telemetry-validation, query-builder, batcher), fakes `InMemoryClickHouseClient` + `InMemoryAlertRepository`. 1 placeholder live test gated `COLBER_LIVE_TESTS=1`.
- **Image Docker** : `colber/observability:dev` (Alpine multi-stage).
...
- **Endpoints** :
  ...
  - gRPC : `colber.negotiation.v1.NegotiationService` (6 RPCs)
...
- **Storage** : Postgres `colber_negotiation` (drizzle 0000) :
...
- **Tests** : 61 unit + integration (strategies × 2, projection rebuild, state machine, JCS+signing, REST lifecycle full via fastify.inject, idempotency replay). 1 placeholder live test gated `COLBER_LIVE_TESTS=1`.
- **Image Docker** : `colber/negotiation:dev` (Alpine multi-stage).
...
- **Endpoints** :
  ...
  - gRPC : `colber.insurance.v1.InsuranceService` (5 RPCs).
...
- **Storage** : Postgres `colber_insurance` (drizzle 0000) :
...
- **Tests** : 54 unit + integration verts (pricing 16, escrow 15, exposure 4, reputation-client 7, REST integration 12). 1 placeholder live test gated `COLBER_LIVE_TESTS=1`.
- **Image Docker** : `colber/insurance:dev` (Alpine multi-stage).
...
- **Endpoints** :
  ...
  - gRPC : `colber.memory.v1.MemoryService`
  - MCP : `memory.store`, `retrieve`, `update`, `share`
- **Storage** :
  - Qdrant 1.15.4 : collection `colber_memories`, vecteurs 768-dim (`nomic-embed-text` via Ollama self-hosted), filtres ACL server-side.
  - Postgres `colber_memory` : `memories`, `memory_versions`, `memory_shares`, `memory_quotas`.
...
- **Image Docker** : `colber/memory:dev` (298 MB).
...
### 2.2 Stack Docker — 16 conteneurs (`docker compose -p colber ps`)

| Conteneur               | Image                                       | Rôle                               | Port hôte       |
| ----------------------- | ------------------------------------------- | ---------------------------------- | --------------- |
| `colber-postgres`       | `postgres:16-alpine`                        | Métadonnées des 4 services         | `15432`         |
| `colber-redis`          | `redis:7-alpine`                            | Cache scoring                      | `16379`         |
| `colber-nats`           | `nats:2.10-alpine`                          | Bus événements (réservé futur)     | `14222`/`18222` |
| `colber-qdrant`         | `qdrant/qdrant:v1.15.4`                     | Vecteurs MEMORY                    | `16333`/`16334` |
| `colber-clickhouse`     | `clickhouse/clickhouse-server:24.10-alpine` | Logs/traces OBSERVABILITY          | `18123`/`19000` |
| `colber-neo4j`          | `neo4j:5-community`                         | Graphe REPUTATION                  | `17474`/`17687` |
| `colber-ollama`         | `ollama/ollama:0.4.7`                       | Embeddings (nomic-embed-text 768d) | `11434`         |
| `colber-prometheus`     | `prom/prometheus:v2.55.1`                   | Métriques                          | `19090`         |
| `colber-grafana`        | `grafana/grafana:11.3.0`                    | Dashboards (admin/colber_dev)      | `13000`         |
| `colber-traefik`        | `traefik:v3.2`                              | Reverse proxy interne              | `18000`/`18080` |
| `colber-agent-identity` | `colber/agent-identity:dev`                 | Service applicatif                 | `14001`/`14002` |
| `colber-reputation`     | `colber/reputation:dev`                     | Service applicatif                 | `14011`/`14012` |
| `colber-memory`         | `colber/memory:dev`                         | Service applicatif                 | `14021`/`14022` |
| `colber-observability`  | `colber/observability:dev`                  | Service applicatif                 | `14031`/`14032` |
| `colber-negotiation`    | `colber/negotiation:dev`                    | Service applicatif                 | `14041`/`14042` |
| `colber-insurance`      | `colber/insurance:dev`                      | Service applicatif                 | `14051`/`14052` |
...
| `colber_reputation`      | reputation     | score_snapshots, feedback_log, merkle_anchors, \_\_drizzle_migrations           |
| `colber_memory`          | memory         | memories, memory_versions, memory_shares, memory_quotas, \_\_drizzle_migrations |
| `colber_observability`   | observability  | alert_rules, \_\_drizzle_migrations                                             |
| `colber_negotiation`     | negotiation    | negotiation_events, negotiation_state, \_\_drizzle_migrations                   |
| `colber_insurance`       | insurance      | policies, escrow_holdings, escrow_events, claims, \_\_drizzle_migrations        |
| `colber` (ClickHouse DB) | observability  | colber_logs, colber_spans (DateTime64 UTC, partitions par jour, TTL 30j)        |
| `colber` (legacy)        | (legacy)       | obsolète, à supprimer                                                           |
...
### 3.2 Tests E2E sur VM β (`COLBER_VM=100.83.10.125 python .tools/e2e_smoke.py`)
...
b4cf39f chore: bootstrap pilotage projet Colber et stack de test conteneurisée
```
...
| 8   | Cohabitation isolée avec ShowWeb3 (option A)                                          | Zéro impact sur projet existant, ports décalés, volumes nommés Colber                  |
...
- 🔴 **PAT GitHub `ghp_lzGq...` exposé en chat** — à révoquer (https://github.com/settings/tokens) et remplacer par un fine-grained PAT scopé `Obi49/Colber` uniquement.
- 🟡 Clés Ed25519 platform et clé AES `MEMORY_ENCRYPTION_KEY` sur la VM dans `/home/claude/Colber/colber-stack/services.env` — fixtures de DEV, à régénérer pour tout autre environnement.
...
**v1 atteinte** : 6 services applicatifs Colber sur la VM β (agent-identity + REPUTATION + MEMORY + OBSERVABILITY + NEGOTIATION + INSURANCE), tous testés en bout en bout. 16 conteneurs Docker isolés sur la VM `100.83.10.125` (cohabitation propre avec ShowWeb3). 21 commits sur `main`.
...
- `colber-stack/services.env` (sur la VM uniquement) — clés platform + KMS.
...
git push "https://x-access-token:${TOKEN}@github.com/Obi49/Colber.git" main
...
# Logs d'un service
python .tools/ssh_run.py --sudo "docker logs --tail=50 colber-reputation"
...
python .tools/ssh_run.py --sudo "cd /home/claude/Colber/colber-stack && docker compose -f docker-compose.yml -f docker-compose.services.yml build <service> && docker compose ... up -d --force-recreate <service>"
```
